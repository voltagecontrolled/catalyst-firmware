...
**Global Settings (Shift + encoders in normal mode):** Default direction (Enc 1), default range (Enc 3), default length (Enc 6), internal BPM (Enc 7).
...
| alpha10 | SHIFT+CHAN hold 2s for Global Settings (replaces broken hold-Shift); short SHIFT+CHAN = Channel Edit; BPM indicator redesigned as ROYGBIV color zones (< 50 = red, 50–79 = orange, 80–99 = yellow, 100–119 = green, 120–149 = blue, 150–179 = teal, 180+ = lavender); Phase Scrub Lock indicator: enc 8 red when locked, blinks red during pickup; Channel Edit length preview on entry (600ms passive display); per-step glide flag editing in armed CV (Glide held + enc N = set/clear); Ratchet Step Editor removed (ratchets editable via main mode hold-step+enc and armed Trigger enc); Gate ratchets added — Gate step encoding: high byte = gate length (0–255 ≈ 0–100%), low byte = ratchet count (0/1 = single gate, 2–8 = N subdivided pulses per step); armed Gate + Glide + enc N = per-step ratchet; unarmed Glide + step held + enc N = per-step ratchet for Gate channels; gate ratchet LED display when Glide held; `current_tag` bumped to 5 |
...
- `VoltSeq::Data::current_tag = 5u` (bumped in alpha10; Gate step encoding changed to high byte = gate length, low byte = ratchet count).
